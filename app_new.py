...
from utils.general import (LOGGER, Profile, check_file, check_img_size,
                           check_imshow, cv2, increment_path,
...
def echo(ws):
    ...
    # parser.add_argument('--weights', nargs='+', type=str,
                        # default=ROOT / 'best_aug.pt', help='model path or triton URL')
    # parser.add_argument('--source', type=str, default='0',
                        # help='file/dir/URL/glob/screen/0(webcam)')
    parser.add_argument('--source', type=str, default='http://192.168.214.240:8080/video', help='file/dir/URL/glob/screen/0(webcam)')
    parser.add_argument('--data', type=str, default=ROOT /
                        'data.yaml', help='(optional) dataset.yaml path')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+',
                        type=int, default=[416], help='inference size h,w')
    parser.add_argument('--conf-thres', type=float,
                        default=0.50, help='confidence threshold')
    ...
    model = DetectMultiBackend(
        opt.weights, device=device, dnn=opt.dnn, data=opt.data, fp16=opt.half)
    ...
    if webcam:
        ...
        dataset = LoadStreams(source, img_size=imgsz,
                              stride=stride, auto=pt, vid_stride=opt.vid_stride)
        bs = len(dataset)
    elif screenshot:
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            annotator = Annotator(
                im0, line_width=opt.line_thickness, example=str(names))
            if len(det):
            ...
            if view_img:
                ...
                cv2.putText(im0, 'Trolley:'+str(len(det)), (50, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2, cv2.LINE_AA)
                ret, buffer = cv2.imencode('.jpg', im0)
                response = json.dumps({
                    "frame": base64.b64encode(buffer).decode("utf-8"),
                    "det": len(det),
                    "timing": f"{dt[1].dt * 1E3:.1f}ms"
                })
                ws.send(response)
        LOGGER.info(
